import type { BetResult, Round, RoundPlayer } from '../types'
 ...
 */
export function calcNassau(round: Round, playerIds: string[], stake: number, useNet: boolean): BetOutcome {
  const entries = playerIds
    .map((id) => round.players.find((rp) => rp.playerId === id))
    .filter((rp): rp is RoundPlayer => !!rp)
  if (entries.length < 2) return emptyOutcome(playerIds, 'Needs at least two golfers.')
  ...
  for (const seg of segments) {
    ...
    const scores = entries.map((rp) => ({
      id: rp.playerId,
      score: strokes(rp, seg.from, seg.to) - (useNet ? rp.handicapSnapshot * seg.allowance : 0),
    }))
